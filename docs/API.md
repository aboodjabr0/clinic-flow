...
| [Medical History](#medical-history) | [api/medical-history.md](api/medical-history.md) | 2 |
...
- `409 Conflict` — duplicate-name conflicts (dental services), doctor↔account link conflicts, duplicate user emails, and last-active-admin protection.
...
| Medical history | view/edit | view only | view/edit |
...
| Users (staff accounts) | full | none | none |
...
Notes/business rules:
- Creating a doctor profile does **not** create a login account. Admin-created login accounts are managed separately via [Users](#users) (`POST /api/users`, optionally with `doctorProfileId`).
...
Known entity types: `Patient`, `DoctorProfile`, `DentalService`, `ClinicSettings`, `Appointment`, `Visit`, `Invoice`, `Payment`, `Auth`, `User`.
...
Admin-only staff account management. This is how real clinics create and maintain login accounts (`AppUser`) for Admins, Doctors, and Receptionists — separate from doctor *profiles* (see [Doctors](#doctors)), which hold clinical/contact info and can optionally be linked to a login via `doctorProfileId`.
...
Notes/business rules:
- `doctorProfileId` is optional even when `role` is `Doctor` — a doctor's clinical profile and login account are independent (see [Doctors](#doctors)). Link them at creation or later via `PUT /api/users/{id}`.
...
- The old password stops working immediately; any existing JWT the user already holds remains valid until it expires (tokens are stateless — see [Auth](#auth)).

Security notes:
- No endpoint ever returns `passwordHash` or a plaintext password — `UserDto` has no password field at all.
- The clinic can never be left with zero active Admins: both `PUT` and `PATCH .../status` block any operation that would remove the last active admin.

Future improvement:
- A `force-password-change` flag (require the user to set a new password on next login) is not implemented in this phase — it would require a schema migration and a login-flow change. Tracked as a follow-up.
